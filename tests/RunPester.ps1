...
Set-Location "C:\OPEN_PROJECTS\ReportCardPS\"
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\ReportCardPS\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\ReportCardPS\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\ReportCardPS\$Directory\*.ps1 -PassThru -ExcludeTag SkipThis
